...
public class LeilaoTest {
	...
	public void naoDeveAceitarMaisDoQue5LancesDeUmMesmoUsuario() {
		...
		leilao.propoe(new Lance(billGates, 6000.0));
		
		leilao.propoe(new Lance(steveJobs, 8000.0));
		leilao.propoe(new Lance(billGates, 9000.0));
		
		leilao.propoe(new Lance(steveJobs, 7700.0));
		leilao.propoe(new Lance(billGates, 8700.0));
		
		leilao.propoe(new Lance(steveJobs, 12000.0));
		leilao.propoe(new Lance(billGates, 31000.0));
		
		//deve ser ignorado
		leilao.propoe(new Lance(steveJobs, 17000.0));
		
		assertEquals(10, leilao.getLances().size());
		assertEquals(31000.0, leilao.getLances().get(leilao.getLances().size()-1).getValor(), 0.00001);
